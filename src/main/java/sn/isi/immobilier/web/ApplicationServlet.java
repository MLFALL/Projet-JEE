...
public class ApplicationServlet extends HttpServlet {
        ...
        try {
            if ("/applications/submit".equals(path)) {
                if (currentUser.getRole() != UserRole.TENANT) {
                    ...
                    writeJson(resp, "{\"error\":\"Vous ne pouvez pas faire cette action\"}");
                    return;
                }
                Long unitId = Long.valueOf(req.getParameter("unitId"));
                Unit unit = unitService.findById(unitId);
                ...
                RentalApplication app = appService.submit(currentUser.getId(), unitId, "Demande via API");
                String json = "{"
                        + "\"id\":" + app.getId() + ","
                        + "\"unitId\":" + (app.getUnit() != null ? app.getUnit().getId() : "null") + ","
            ...
            if (currentUser.getRole() == UserRole.TENANT) {
                ...
                list = appService.listAll();
            }

            StringBuilder sb = new StringBuilder("[");
            for (int i = 0; i < list.size(); i++) {
                RentalApplication a = list.get(i);
                sb.append("{")
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String path = req.getServletPath();
        try {
            if ("/applications/approve".equals(path) || "/applications/reject".equals(path)) {
                if (currentUser.getRole() != UserRole.OWNER && currentUser.getRole() != UserRole.ADMIN) {
                    resp.setStatus(HttpServletResponse.SC_FORBIDDEN);
                    writeJson(resp, "{\"error\":\"Vous n'avez pas les droits\"}");
                    return;
                }
                Long appId = Long.valueOf(req.getParameter("applicationId"));
                RentalApplication app;
                if ("/applications/approve".equals(path)) {
                    app = appService.approve(appId, currentUser.getId());
                    // si transientLease existante, on peut signaler
                    String json = "{ \"message\":\"Application approuvée\", \"applicationId\":" + app.getId();
                    if (app.getTransientLease() != null) {
                        json += ", \"leaseTempId\":null"; // lease n'est pas persistant côté transient, on ne force rien
                    }
                    json += " }";
                    writeJson(resp, json);
                } else {
                    String reason = req.getParameter("reason");
                    if (reason == null) reason = "Aucune raison spécifiée";
                    app = appService.reject(appId, currentUser.getId(), reason);
                    writeJson(resp, "{\"message\":\"Application rejetée\",\"applicationId\":" + app.getId() + "}");
                }
                return;
            }
